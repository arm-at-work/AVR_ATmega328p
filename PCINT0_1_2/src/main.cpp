 ...
 * sucessivamente.
 *
 * No presente arquivo queremos uma interrupção para os pino PB1, PC5 e PD4, sendo assim, setamos o bit PCINT1 do registrador PCMSK0,
 * o bit PCINT13 do registrador PCMSK1 e o bit PCINT20 do registrador PCMSK2, além de setarmos os bit PCIE0, PCIE1 e PCIE2 do registrador PCICR.
 */
...
int main(void) {
	pins.PCINT0Config(); // inicializa a interrupção por mudança no pino
	pins.PCINT1Config(); // inicializa a interrupção por mudança no pino
	pins.PCINT2Config(); // inicializa a interrupção por mudança no pino
	pins.ledsConfig(); // configura pino de led
	sei();
...
ISR(PCINT0_vect) {
	PCIFR &= ~(1 << PCIF0); // seta flag de interrupção em andamento, para impedir que muitas interrupções sejam geradas
	if (!(PINB & (1 << PINB1))) { // verifica se a mudança foi para estado lógico baixo
		PORTB ^= (1 << PB5); // inverte estado lógico do pino
	}
	PCIFR |= (1 << PCIF0); // limpa flag de interrupção
}
...
ISR(PCINT1_vect) {
	...
	if (!(PINC & (1 << PINC5))) { // verifica se a mudança foi para estado lógico baixo
		PORTB ^= (1 << PB4); // inverte estado lógico do pino
	}
	PCIFR |= (1 << PCIF1); // limpa flag de interrupção
}

ISR(PCINT2_vect) {
	PCIFR &= ~(1 << PCIF2); // seta flag de interrupção em andamento, para impedir que muitas interrupções sejam geradas
	if (!(PIND & (1 << PIND4))) { // verifica se a mudança foi para estado lógico baixo
		PORTB ^= (1 << PB3); // inverte estado lógico do pino
	}
	PCIFR |= (1 << PCIF2); // limpa flag de interrupção
}
