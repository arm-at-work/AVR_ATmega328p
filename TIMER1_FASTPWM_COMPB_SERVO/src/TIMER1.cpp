...
void TIMER1::TIMER1Config(void) {
	TCCR1A = ((TCCR1A & ~(1 << COM0A1)) | (1 << COM0A0)); // 0C1A/OC1B = 0 na comparação, em 1 no final do período
	TCCR1A |= (0b11 << WGM10); // timer em modo FAST PWM
	TCCR1B |= (0b11 << WGM12); // timer em modo FAST PWM
	TCCR1B = ((TCCR1B & ~(1 << CS12)) | (0b11 << CS10)); // prescaler de 64
	OCR1A =  1200; // atribui valor calculado ao registrador de comparação do timer1
	OCR1B =  4999; // atribui valor calculado ao registrador de comparação do timer1
}
