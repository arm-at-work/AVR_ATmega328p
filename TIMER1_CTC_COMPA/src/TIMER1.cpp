...
void TIMER1::TIMER1Config(void) {
	TCCR1A &= ~(0b11 << WGM10); // timer em modo CTC
	TCCR1B = ((TCCR1B & ~(1 << WGM13)) | (1 << WGM12)); // timer em modo CTC
	TCCR1B = ((TCCR1B & ~(0b11 << CS10)) | (1 << CS12)); // prescaler de 256
	OCR1A =  31249; // atribui valor calculado ao registrador de comparação do timer1
