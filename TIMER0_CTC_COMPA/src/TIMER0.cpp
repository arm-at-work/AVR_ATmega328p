...
void TIMER0::TIMER0Config(void) {
	TCCR0A = ((TCCR0A & ~(1 << WGM00)) | (1 << WGM01)); // timer em modo CTC
	TCCR1B &= ~(1 << WGM02); // timer em modo CTC

	TCCR0B = ((TCCR0B & ~(1 << CS01)) | (1 << CS02 | 1 << CS00)); // prescaler de 1024
	OCR0A =  250; // atribui valor calculado ao registrador de comparação do TIMER0
