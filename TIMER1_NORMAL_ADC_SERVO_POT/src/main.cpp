...
/**
 * Method initializa application.
 * @param void Method does not receive params
 * @return int
 */
int main(void) {
	pins.pinsConfig(); // método configura pino do servo
	timer1.TIMER1Config(); // método configura TIMER1
	adc.ADCConfig(); // método configura ADC
	sei();
...
ISR(TIMER1_COMPA_vect) {
	PORTB &= ~(1 << PB1); // estado lógico do pino baixo
}
...
ISR(TIMER1_COMPB_vect) {
	PORTB |= (1 << PB1);
	TCNT1 = 0; // seta contador do TIMER1
}
 ...
 */
ISR(ADC_vect) {
	OCR1A = (ADCW / 2) + 124; // atualiza valor do comparador do timer, segundo valor lido do potenciômetro
}
