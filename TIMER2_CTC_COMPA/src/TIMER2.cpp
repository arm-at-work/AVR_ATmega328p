...
void TIMER2::TIMER2Config(void) {
	TCCR2A = ((TCCR2A & ~(1 << WGM20)) | (1 << WGM21)); // timer em modo CTC
	TCCR2B &= ~(1 << WGM22); // timer em modo CTC

	TCCR2B |= (0b111 << CS20); // prescaler de 1024
	OCR2A =  250; // atribui valor calculado ao registrador de comparação do TIMER2
	TIMSK2 |= (1 << OCIE2A); // habilita interrupção por comparação
}
