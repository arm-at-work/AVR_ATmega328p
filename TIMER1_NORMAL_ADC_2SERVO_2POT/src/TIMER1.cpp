...
/**
 * Method implements TIMER1 configurations
 */
void TIMER1::TIMER1Config(void) {
	TCCR1A &= ~(0b11 << WGM10); // TIMER1 on NORMAL mode
	TCCR1B &= ~(0b11 << WGM12); // TIMER1 on NORMAL mode
	TCCR1B = ((TCCR1B & ~(1 << CS12)) | (0b11 << CS10)); // prescaler de 64
	TIMSK1 |= (1 << OCIE1A | 1 << OCIE1B | 1 << TOIE1); // habilita interrupção por estouro do contador
	OCR1A = 60660; // responsável pelo dutty cycle A do pulso
	OCR1B = 60660; // responsável pelo dutty cycle B do pulso
	TCNT1 = this->tc1Count; // seta contador do TIMER1
}
